export const contactInfo = {
  ...
  linkedin: 'https://www.linkedin.com/in/dev-manuel-sereno',
  linkedinHandle: '@dev-manuel-sereno',
...
export type Skill = {
  ...
  logo: string
  /**
   * Onde a tecnologia é efetivamente usada. Uma linha curta e falsificável,
   * que substitui a nota de proficiência autoatribuída: um número de 0 a 100
   * não pode ser verificado por ninguém, "uso na PASS, em produção" pode.
   */
  context: string
...
/**
 * A ordem desta lista é a ordem exibida — na grade de habilidades e na esteira
 * de logos. Ela é o único mecanismo de hierarquia da seção, então mexer aqui
 * muda o que o visitante lê primeiro.
 *
 *   base da web  →  stack de produção  →  ferramentas  →  back-end e dados
 *
 * Cada linha da grade de 3 colunas se lê sozinha, sem precisar de cabeçalho.
 */
export const allSkills: Skill[] = [
  ...
  // O stack de produção
  {
    ...
    context: 'Uso na PASS, em produção',
    summary:
  ...
  // Ferramentas do dia a dia
  {
    ...
    context: 'Usei na identidade da Giuliana Ragno',
    summary:
...
/** Sequência de logos usada na esteira animada, na mesma ordem de `allSkills`. */
export const skillMarquee: { name: string; logo: string }[] = allSkills.map(
  (s) => ({ name: s.name, logo: s.logo }),
)
