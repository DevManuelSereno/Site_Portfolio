...
const Skills = () => {
  // Known technologies with proper icons
  const knownSkills = [
    { 
      name: 'HTML', 
      icon: (
  ...
  const learningSkills = [
    ...
    { 
      ...
      icon: (
        <svg viewBox="0 0 24 24" className="w-8 h-8" fill="currentColor">
          <path d="M11.998 24c-.321 0-.641-.084-.922-.247l-2.936-1.737c-.438-.245-.224-.331-.08-.381.585-.203.703-.25 1.328-.604.065-.037.151-.023.218.017l2.256 1.339c.082.045.197.045.272 0l8.795-5.076c.082-.047.134-.141.134-.238V6.921c0-.099-.053-.192-.137-.242l-8.791-5.072c-.081-.047-.189-.047-.271 0L2.46 6.68c-.085.05-.139.143-.139.242v10.152c0 .097.054.189.139.235l2.409 1.392c1.307.654 2.108-.116 2.108-0.89V7.787c0-.142.114-.253.255-.253h1.115c.139 0 .255.112.255.253v10.021c0 1.745-.95 2.745-2.604 2.745-.508 0-.909 0-2.026-.551L2.28 18.675c-.57-.329-.922-.944-.922-1.604V6.921c0-.659.353-1.275.922-1.603l8.795-5.082c.557-.315 1.296-.315 1.848 0L21.72 5.318c.571.328.924.944.924 1.603v10.15c0 .659-.353 1.275-.924 1.604l-8.795 5.082c-.28.163-.599.247-.927.247zm2.725-6.961c-3.855 0-4.66-1.767-4.66-3.252 0-.141.113-.254.255-.254h1.136c.126 0 .232.093.25.217.17 1.145.677 1.724 3.019 1.724 1.858 0 2.651-.421 2.651-1.408 0-.569-.224-.991-3.106-1.275-2.409-.237-3.897-.771-3.897-2.699 0-1.778 1.497-2.837 4.009-2.837 2.822 0 4.215.98 4.389 3.086a.257.257 0 01-.098.206.26.26 0 01-.158.05h-1.136a.254.254 0 01-.248-.202c-.274-1.217-.940-1.607-2.749-1.607-2.024 0-2.258.707-2.258 1.236 0 .644.278.832 3.007 1.195 2.705.359 3.999.862 3.999 2.760 0 1.91-1.592 3.020-4.370 3.020z"/>
        </svg>
      ), 
      color: 'bg-green-600' 
    },
  ...
  const SkillCard = ({ skill, isLearning = false }: { skill: Skill; isLearning?: boolean }) => (
    <div 
      className={`${skill.color} rounded-lg p-6 text-white flex flex-col items-center justify-center aspect-square hover:scale-105 transition-transform duration-300 relative group cursor-pointer`}
    >
      {isLearning && (
        <div className="absolute -top-1 -right-1 bg-yellow-400 text-yellow-900 rounded-full p-1 text-xs font-bold animate-pulse">
          📚
        </div>
      )}
      ...
      {isLearning && (
        <div className="absolute -bottom-10 left-1/2 transform -translate-x-1/2 text-white px-2 py-1 rounded text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap z-10" style={{backgroundColor: '#2A2D32'}}>
          Em estudo
        </div>
      )}
    </div>
